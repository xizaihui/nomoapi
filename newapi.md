...
### 2026-03-17 — P4-4: 移除 Playground + Semi 运行时脱离 (commit: `05189132`)

#### 删除的文件（39 files, -5,463 lines）
- `components/playground/` — 17 个组件文件（ChatArea, ConfigManager, SettingsPanel 等）
- `hooks/playground/` — 6 个 hook 文件
- `contexts/PlaygroundContext.jsx`
- `constants/playground.constants.js`
- `pages/Playground/index.jsx`

#### 代码修改
- `App.jsx` — 移除 `/console/playground` 路由
- `SiderBar.jsx` — 移除「操练场」侧栏菜单项
- `PageLayout.jsx` — 移除 playground 路径判断
- `compat/index.js` — Chat 真实导出改为 `() => null` stub
- `vite.config.js` — 移除 vite-plugin-semi + deep path aliases + __real alias
- `index.jsx` — 移除 semi.css 导入 + locale 导入 + useTranslation
- `index.css` — 移除 `@layer semi`
- `render.jsx` — 移除 playground icon case
- `api.js` / `utils.jsx` — 内联 MESSAGE_ROLES / THINK_TAG_REGEX
- `useSidebar.js` / `SettingsSidebarModulesAdmin.jsx` / `NotificationSettings.jsx` — 移除 playground 配置
- `CodeViewer.jsx` 移到 `components/common/`（NotificationSettings 仍使用）

#### 构建效果
| 指标 | P4 前 | P4 后 | 变化 |
|------|-------|-------|------|
| Semi JS chunk | 1,369KB | 240KB | **-82%** |
| Semi CSS | 570KB | 0.5KB | **-99.9%** |
| Total CSS | 1.23MB | 145KB | **-88%** |
| 源代码 | ~102K lines | ~97K lines | -5,463 lines |
| 构建时间 | 2m15s | 1m50s | -18% |

#### 验证
- Headless Chrome 测试 4 条路由（/, /login, /register, /pricing）：✅ 零错误
- Docker 本机部署成功
- Chat 页面（iframe）和 Chat2Link 不受影响（它们不依赖 Playground）
- ChatsSetting（管理后台聊天设置）不受影响
