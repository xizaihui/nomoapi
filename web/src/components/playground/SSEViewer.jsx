...
const SSEViewer = ({ sseData }) => {
  ...
  const renderSSEItem = (item) => {
    if (item.isDone) {
      return (
        <div className='flex items-center gap-2 p-3 bg-muted/30 dark:bg-foreground/20 rounded-lg'>
          <CheckCircle size={16} className='text-foreground/80' />
          <Typography.Text className='text-foreground/80 font-medium'>
            {t('流式响应完成')} [DONE]
          </Typography.Text>
        </div>
      );
    ...
    if (item.error) {
      return (
        <div className='space-y-2'>
          <div className='flex items-center gap-2 p-3 bg-muted/30 dark:bg-foreground/20 rounded-lg'>
            <XCircle size={16} className='text-foreground/80' />
            <Typography.Text className='text-foreground/80'>
              {t('解析错误')}: {item.error}
            </Typography.Text>
          </div>
          <div className='p-3 bg-muted/60 dark:bg-foreground/90 rounded-lg font-mono text-xs overflow-auto'>
            <pre>{item.raw}</pre>
          </div>
        </div>
      );
    ...
    return (
      <div className='space-y-2'>
        ...
        <div className='relative'>
          <pre className='p-4 bg-foreground text-muted-foreground/20 rounded-lg overflow-auto text-xs font-mono leading-relaxed'>
            {JSON.stringify(item.parsed, null, 2)}
          </pre>
          <Button
            icon={<Copy size={12} />}
            ...
            className='absolute top-2 right-2 !bg-foreground/90/80 !text-muted-foreground/40 hover:!bg-foreground/80'
          />
        </div>
    ...
    <div className='h-full flex flex-col bg-muted/30 dark:bg-foreground/50 rounded-lg'>
      ...
      <div className='flex-1 overflow-auto p-4'>
        <Collapse
          ...
          accordion={false}
          className='bg-white dark:bg-foreground/90 rounded-lg'
        >
            ...
            <Collapse.Panel
              ...
              header={
                <div className='flex items-center gap-2'>
                  ...
                  {item.isDone ? (
                    <span className='text-foreground/80 font-medium'>[DONE]</span>
                  ) : item.error ? (
                    <span className='text-foreground/80'>{t('解析错误')}</span>
                  ) : (
                    <>
